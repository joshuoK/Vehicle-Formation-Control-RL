"""
train_rl.py (v8 — Zero-Collision + Full-Detection)
...
# -- Coverage grid ----------------------------------------------------------
GRID_N = 20                  # v8: finer 20×20 grid (was 10) for better exploration guidance
...
# -- Curriculum -------------------------------------------------------------
# v8: wider stages give more time to consolidate at each difficulty level
CURRICULUM_STAGES = [
    (0,           3),
    (1_500_000,   5),
    (3_500_000,   7),
...
# v9: obstacle curriculum — ramp up map complexity alongside person count
# Starts with 2 obstacles so agent learns basic avoidance before full clutter
OBSTACLE_CURRICULUM_STAGES = [
    (0,           2),
    (1_500_000,   4),
    (3_500_000,   6),
    (6_000_000,   7),
...
EVAL_ACTIVE_OBSTACLES = OBSTACLE_CURRICULUM_STAGES[-1][1]

# -- Reward shaping (v8 — zero-collision + full-detection focus) -----------
#
# DESIGN PRINCIPLES:
...
#
# v8 changes vs v7:
#   R_COLLISION:        -20  → -350  (dominant signal; zero collisions is hard constraint)
#   R_PERSON_FOUND:     +10  → +40   (dominant signal; finding everyone is the goal)
#   R_SPIN_PENALTY:    -0.5  → -0.25 (aggressive anti-twirling for stability)
#   R_EARLY_COMPLETION: +5   → +15   (stronger incentive to complete the mission)
#   Formation threshold: 0.5 → 1.5m  (achievable threshold; rewards formation keeping)
#
R_PERSON_FOUND          =   40.0
R_COLLISION             =  -350.0
...
LEARNING_RATE  = cosine_lr_schedule(3e-4, final=1e-5)  # v9: floor at 1e-5, never dies to zero
N_STEPS        = 2048         # v8: longer rollouts → better LSTM credit assignment (was 1024)
BATCH_SIZE     = 256          # Fits comfortably in 8GB VRAM with LSTM
N_EPOCHS       = 6
GAMMA          = 0.995
GAE_LAMBDA     = 0.95
...
# -- Training schedule ------------------------------------------------------
TOTAL_TIMESTEPS  = 12_000_000  # v9 resume: 4M extra steps beyond v8's 8M
...
class SARGymnasiumWrapper(gym.Env):
    ...
    def _shape_reward(
        ...
        # ── 3. FORMATION KEEPING ──
        if info["form_dev_mean"] < 1.5:  # v8: achievable threshold (was 0.5)
...
class CurriculumCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if target_stage != self._current_stage:
            ...
            n_obstacles = OBSTACLE_CURRICULUM_STAGES[target_stage][1]
...
def train(device: str = "auto", resume: bool = False) -> None:
    print("=" * 65)
    print("  SAR SWARM — RecurrentPPO + LSTM (v8 — Zero-Collision + Full-Detection)")
    print("=" * 65)
    print(
    ...
    print("  Curriculum:")
    for i, ((thr, n), (_, o)) in enumerate(zip(CURRICULUM_STAGES, OBSTACLE_CURRICULUM_STAGES)):
    ...
    train_env.env_method("set_n_active_obstacles", OBSTACLE_CURRICULUM_STAGES[0][1])
    ...
    eval_env.env_method("set_n_active_obstacles", N_OBS)
