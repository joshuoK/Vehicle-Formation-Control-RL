...
COVER_COLOR        = "#00e676"   # green floor tiles for discovered area
BG_COLOR           = "#0a0a14"

FRAME_SKIP = 5          # subsample: render every Nth step (lower = smoother but larger MP4)
VIDEO_FPS  = 24         # frames per second in output MP4
ROBOT_Z    = 0.30       # top of robot sensor box [m] — used for trails/drop-lines
TRAIL_ALPHA = 0.45
COVER_ALPHA = 0.90
...
def _draw_coverage(ax: Axes3D, coverage_grid: np.ndarray) -> None:
    """Draw covered cells as a coloured floor surface using plot_surface.
    Using a single surface avoids the painter-algorithm z-sort problem that
    causes Poly3DCollection tiles to overdraw robots and FOV cones."""
    import matplotlib.colors as mcolors

    coverage_2d = coverage_grid.reshape(T.GRID_N, T.GRID_N)

    # Build per-face RGBA colour array: (GRID_N, GRID_N, 4)
    rgba_on  = np.array(mcolors.to_rgba(COVER_COLOR, alpha=COVER_ALPHA))
    rgba_off = np.array([0.0, 0.0, 0.0, 0.0])          # fully transparent
    # meshgrid rows → Y axis, cols → X axis, so transpose coverage
    facecolors = np.where(coverage_2d.T[:, :, np.newaxis], rgba_on, rgba_off)

    x = np.linspace(0, MAP_W, T.GRID_N + 1)
    y = np.linspace(0, MAP_H, T.GRID_N + 1)
    X, Y = np.meshgrid(x, y)
...
def _draw_obstacle_cylinder(ax: Axes3D, ox: float, oy: float,
    ...
    theta  = np.linspace(0, 2 * np.pi, 48)   # more segments → smoother circles
    z_cyl  = np.linspace(0.0, height, 6)
...
def _draw_fov_arc(ax: Axes3D, rx: float, ry: float, rtheta: float,
    ...
    arc_z = [0.25] * len(arc_x)
    ax.plot3D(arc_x, arc_y, arc_z, color=color, alpha=0.35, lw=1.0)


def _draw_robot(ax: Axes3D, rx: float, ry: float, rtheta: float,
                color: str, label: str) -> None:
    """Draw a simplified ground robot: flat box body, dark tracks, small sensor box."""
    from mpl_toolkits.mplot3d.art3d import Poly3DCollection

    c, s = np.cos(rtheta), np.sin(rtheta)

    def w(lx: float, ly: float, lz: float):
        """Rotate local (lx,ly) by rtheta and translate to world (rx,ry)."""
        return (rx + lx * c - ly * s,
                ry + lx * s + ly * c,
                lz)

    def box(lx0, lx1, ly0, ly1, lz0, lz1):
        """Return the 6 faces of an axis-aligned box in local coords → world."""
    ...
    # ── Tracks (dark, slightly wider & lower than body) ───────────────────
    ...
    # ── Sensor box on top (slightly forward, same colour as body) ─────────
    ax.add_collection3d(Poly3DCollection(
    ...
    # ── Heading arrow ──────────────────────────────────────────────────────
    ax.plot3D(
        [rx, rx + 0.7 * c], [ry, ry + 0.7 * s], [0.22, 0.22],
        color=color, lw=1.8,
    )

    # ── Drop line to floor ─────────────────────────────────────────────────
    ax.plot3D([rx, rx], [ry, ry], [0, ROBOT_Z],
              color=color, alpha=0.20, lw=0.7, ls="--")
...
def save_episode_mp4(
    ...
    try:
        ...
        anim.save(str(output_path), writer=writer, dpi=120)  # 16×9 @ 120dpi = 1920×1080
