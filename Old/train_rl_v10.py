"""
train_rl_v10.py — SAR Swarm | Fixed 3-Person Mission

Goal: master finding exactly 3 persons with 2 obstacles in a 20×20 map.
      Difficulty is fixed from the start — no curriculum scaling up.

Changes from v9:
  ✓ FIXED DIFFICULTY: 3 persons, 2 obstacles for the entire run.
      Removes all multi-stage curriculum complexity and dead code paths.
  ✓ PRE-TRAINING PHASE (first PRE_TRAIN_STEPS): collision penalty is soft
      (-5 vs -50) so the warm-started agent can explore freely while
      re-adapting to the v10 obs space before full penalties are enforced.
  ✓ FAST-COMPLETION BONUS raised from 150 → 350 — speed is now a dominant
      signal once the agent reliably finds all 3 persons.
  ✓ BOOL COVERAGE GRID: float32 → bool saves 75% memory, speeds up
      comparison operations across 400 cells every step.
  ✓ SINGLE PASS COVERAGE UPDATE: uncovered cells computed once, reused
      across all robots — eliminates redundant per-robot mask cloning.
  ✓ CACHED UNCOVERED MASK: _update_sensor_coverage stores the post-step
      uncovered mask on self so _nearest_uncovered_info reads it directly
      without recomputing.
  ✓ PRE-ALLOCATED OBS BUFFER: avoids np.concatenate allocation each step.
  ✓ SIMPLIFIED CALLBACKS: Stage-6 / all-stage eval complexity removed;
      standard EvalCallback replaces StageAwareEvalCallback.
  ✓ WARM-START from checkpoints/best_model (v9 peak checkpoint).
      Policy weights transferred; training counters reset to step 0.
"""
...
def _enable_live_train_logging(log_filename: str = "train_log_v10.txt") -> None:
    global _LOG_FILE_HANDLE, _ORIG_STDOUT, _ORIG_STDERR
    if _LOG_FILE_HANDLE is not None:
        return
...
# -- Mission (fixed — no curriculum scaling) --------------------------------
ACTIVE_PERSONS   = 3          # exactly 3 persons to find
ACTIVE_OBSTACLES = 2          # exactly 2 obstacles to avoid
...
# -- Training schedule ------------------------------------------------------
TOTAL_TIMESTEPS = 30_000_000   # fixed 3-person task needs far fewer steps than v9
CHECKPOINT_FREQ =    500_000
EVAL_FREQ       =     50_000
N_EVAL_EPISODES =        5

# -- File paths -------------------------------------------------------------
# Load from v9's best checkpoint; write to v10-specific directories.
V9_CHECKPOINT_PATH    = "checkpoints/best_model"   # v9 peak checkpoint (load)
MODEL_SAVE_PATH       = "ppo_swarm_agent_v10"      # final model save
TENSORBOARD_LOG       = "./ppo_sar_tensorboard_v10_restarted/"
...
class SARGymnasiumWrapper(gym.Env):
    ...
    def _apply_curriculum(self) -> None:
        ...
        for i, person in enumerate(self._env.persons):
            if i >= ACTIVE_PERSONS and not person.detected:
        ...
        self._env._n_active_obstacles = ACTIVE_OBSTACLES
    ...
    def render(self) -> None:
        ...
        self._fig.suptitle(
            f"SAR v10 [RL+APF]  t={self._env.t:.1f}s  "
            f"Active={ACTIVE_PERSONS}  "
...
def _find_latest_checkpoint(checkpoint_dir: str):
    ...
    checkpoint.  Checks both the on-termination save (ppo_swarm_agent_v10.zip)
    and periodic checkpoints (ppo_sar_v10_*_steps.zip), picks whichever was
    ...
    for z in glob.glob(os.path.join(checkpoint_dir, "ppo_sar_v10_*_steps.zip")):
...
def train(
    ...
    Args:
        ...
        resume:              If True, load v10 checkpoint and continue.
    ...
    """
    print("=" * 68)
    print("  SAR SWARM — RecurrentPPO + LSTM (v10 — Fixed 3-Person Mission)")
    print("=" * 68)
    ...
    print(f"  Mission      : {ACTIVE_PERSONS} persons, {ACTIVE_OBSTACLES} obstacles (fixed)")
    ...
    if resume and os.path.exists(_resume_model + ".zip"):
        ...
        # Fresh v10 model with v9 weights transferred
    ...
    print("  TensorBoard: tensorboard --logdir ppo_sar_tensorboard_v10\n")
    t0 = time.time()
    ...
    try:
        model.learn(
            ...
            tb_log_name=(None if (resume and resume_tensorboard) else "RecurrentPPO_SAR_v10"),
        )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="RecurrentPPO v10 — fixed 3-person SAR mission."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Resume from v10 checkpoint (checkpoints_v10/best_model.zip)",
    )
    parser.add_argument(
        "--no-warm-start", action="store_true",
        help="Disable v9 weight transfer; train from scratch instead",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Path to model zip to resume from, without .zip extension "
             "(default: ppo_swarm_agent_v10)",
    )
    parser.add_argument(
        "--vecnorm", "-v", default=None,
        help="Path to VecNormalize pkl to load on resume "
             "(default: checkpoints_v10/vecnormalize.pkl)",
    )
    return parser.parse_args()


if __name__ == "__main__":
    _enable_live_train_logging("train_log_v10.txt")
    args = _parse_args()
    ...
    if args.mode == "check":
        ...
        train(
            device=args.device,
            resume=args.resume,
            warm_start_from_v9=not args.no_warm_start,
