...
class Robot:
    ...
    def __init__(self, x: float, y: float, theta: float,
        ...
        self.rid        = rid
        self.sensor_off = sensor_off   # FOV heading offset [rad]
        self.v          = 0.0
        self.w          = 0.0
        self.found_ids  = set()        # person IDs detected by this robot
        self.trail: list = []          # [(x, y), ...]
...
class SharedObstacleMap:
    ...
    def update(self, ox: float, oy: float, rad: float) -> bool:
        ...
        (useful for counting discoveries per step).
        """
        k = self._key(ox, oy)
    ...
    def all(self) -> list:
        """Return all known obstacles as a list of (ox, oy, rad) tuples."""
...
def sense_obstacles(robot: Robot,
    ...
    Detection criterion:
      ∥robot.pos − obstacle_centre∥  <  OBS_SENSE_RANGE + obstacle_radius
      AND  angle_to_centre within ±FOV_ANG/2 of sensor heading

    Adding the obstacle radius to the range ensures even large obstacles
    at the periphery of sensor range are caught before the robot hits them.

    Returns the number of newly discovered obstacles this call.
    """
    sensor_hdg = wrap(robot.theta + robot.sensor_off)
    half       = FOV_ANG / 2.0
    new        = 0
    for ox, oy, rad in true_obstacles:
        diff  = np.array([ox, oy]) - robot.pos
        dist  = float(np.linalg.norm(diff))
        if dist > OBS_SENSE_RANGE + rad:
            continue
        angle = float(np.arctan2(diff[1], diff[0]))
        if abs(wrap(angle - sensor_hdg)) <= half:
            if shared_map.update(ox, oy, rad):
                new += 1
    return new


def fov_detect_persons(robot: Robot, persons: list) -> int:
    """
    Mark persons inside the robot's FOV cone as detected.
    Returns the count of new detections this call.
    """
    sensor_hdg = wrap(robot.theta + robot.sensor_off)
    half       = FOV_ANG / 2.0
    new        = 0
...
def in_collision(robot: Robot, obstacles: list) -> bool:
    ...
    hits_obstacle = any(
        np.hypot(robot.x - ox, robot.y - oy) < rad + R_BODY
        for ox, oy, rad in obstacles
    )
    touches_wall = (
        robot.x <= R_BODY
        or robot.x >= (MAP_W - R_BODY)
        or robot.y <= R_BODY
        or robot.y >= (MAP_H - R_BODY)
    )
    return hits_obstacle or touches_wall
...
class SAREnvironment:
    ...
    def __init__(self, seed: int = 202):
        ...
        wpts_arr       = [np.array(w) for w in self.waypoints]
    ...
    def reset(self) -> np.ndarray:
        ...
        # Track explored cells to reward map coverage
    ...
    def step(self, actions) -> tuple:
        ...
        for robot in self.robots:
            self.total_found += fov_detect_persons(robot, self.persons)
        ...
        step_pdev = min(
            point_to_segment_dist(leader.pos, a, b)
            for a, b in self.path_segs
        )
    ...
    def _build_obs(self) -> np.ndarray:
        """
        Assemble the normalised flat observation vector.
        All values lie in approximately [−1, 1].
        """
    ...
    def _update_coverage(self, robot) -> int:
        ...
        """
        grid_cols = self.coverage_grid.shape[1]
        grid_rows = self.coverage_grid.shape[0]
        new_cells = 0
        
        # Find which grid cell(s) the robot covers
        robot_cell_x = int(robot.x / COV_GRID_RES)
        robot_cell_y = int(robot.y / COV_GRID_RES)
        
        # Mark cell and neighbors within coverage threshold
        for dx in [-1, 0, 1]:
            for dy in [-1, 0, 1]:
                cx = robot_cell_x + dx
                cy = robot_cell_y + dy
                if 0 <= cx < grid_cols and 0 <= cy < grid_rows:
                    if not self.coverage_grid[cy, cx]:
                        self.coverage_grid[cy, cx] = True
                        new_cells += 1
